...
也正因为如此，像蚂蚁金服的SEATA（ https://github.com/seata/seata ）那样的分布式事务框架还是太硬、太重了，因为它还是假设分布式服务底层基于mysql数据库，任何在数据库驱动层做文章来解决之前XA锁定资源时间过长导致的性能问题。
SEATA也许是一个更好的XA解决方案，但它终将不是弹性事务，因此跟本项目不是一个类型的，不在一条赛道上。
