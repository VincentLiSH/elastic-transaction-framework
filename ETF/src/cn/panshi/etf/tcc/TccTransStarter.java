...
public class TccTransStarter<T_tcc_trans_enum_type extends Enum<T_tcc_trans_enum_type>> {
	...
	Class<T_tcc_trans_enum_type> tccTransEnumType = (Class<T_tcc_trans_enum_type>) ((ParameterizedType) getClass()
	...
	public final void prepareTccTrans(TccTransPrepareStatement ps) throws EtfTccException4PrepareStage {
		String bizId = null;
		try {
			ps.doPrepare();//会被TCC aop拦截 设置TCC交易类型上下文到ThreadLocal，供下面的代码做交易类型检查
		} catch (EtfTccException4ReturnBizCode e) {//AOP抛出此异常 隐式的返回bizId，避免要求子类显式的返回  造成框架侵入性过高和使用不便！
			bizId = e.getBizId();
		}
		...
		try {
			...
			throw new EtfTccException4PrepareStage("准备启动的事务类型[" + tccEnumValue + "]不是一个合法的" + tccTransEnumType.getName()
		...
		if (StringUtils.isBlank(bizId)) {
			throw new EtfTccException4PrepareStage(
					"TCC交易[" + tccTransEnumType.getName() + "." + tccEnumValue + "]未返回bizId业务流水号，请确保正确实现了TCC交易API的回调！");
		}

		TCC_ENUM_TYPE_OF_JUST_PREPARED_TRANS.remove();
		TCC_ENUM_VALUE_OF_JUST_PREPARED_TRANS.remove();

		etfTccDao.addEtfTccTransPrepareList(tccTransEnumType.getName(), bizId, tccEnumValue);

		logger.debug("TCC交易[" + tccTransEnumType.getName() + "." + tccEnumValue + "]准备成功");
	}

	public final void startTccTransList() {
